...
public class UserServiceImplemented implements IUserService
{
 ...
 static org.slf4j.Logger logger =  LoggerFactory.getLogger(UserServiceImplemented.class);
	...
	//	if (user.getEmail() == null) 
        ...
        javaMailSender.send(MessageUtility.verifyUserMail(registrationDto.getEmail(), token , UserUtility.VALIDATE_MAIL_TEXT));
...
//--------------------------------------------------------------------------------------//
	public Response validateUser(String token)
	{
		...
		{
			return new Response(UserUtility.HTTP_STATUS_BAD_REQUEST, "account not verfied", UserUtility.EMAIL_NOT_VERFIED);
	...
	{	
		...
		{	
			//matches() method of PasswordEncoder interface matches the raw password entered by the user with the encoded password present in the DB.
			if ( user.getEmail().equals(logindto.getEmail() )
					&& config.encoder().matches( logindto.getPassword()/* raw password entered inside the DTO. */, user.getPassword()/* this is encoded password*/ ) )
			{
				// encode the user
		...
		return new Response(UserUtility.HTTP_STATUS_BAD_REQUEST, "User LOGIN failed. ", UserUtility.LOGIN_NOT_SUCCESSFULLY);
	...
	public Response forgetPassword(ForgetPasswordDto fgdto)
	{
		...
		else 
		{
			...
			javaMailSender.send(MessageUtility.verifyUserMail(fgdto.getEmail(), token1, UserUtility.VALIDATE_MAIL_TEXT)); 
	...
	{
		//extracting the email id of the user from the given token and storing it into a variable.
		...
		//if provided password and confirm password are entered equal by the user then enter inside the loop.
		if ( setPasswordDto.getPassword().equals( setPasswordDto.getConfirmPassword() ) ) 
		{ 
			
			//setting userz new password.
			//using PasswordEncoder.encode() method to store the password in the encrypted form in our DB.
			...
			//updateuser(updateuser, userEmail); //method called to update the user, as changes have been made in the password.
			
			//returning the response class .
			return new Response(UserUtility.HTTP_STATUS_OK, "Dear User Your password has been successfully changed.", UserUtility.PASSWORD_CHANGED);

		}
		else 
		{
			
			return new Response(UserUtility.HTTP_STATUS_BAD_REQUEST , "Sorry some error occured. Password not Changed.", UserUtility.PASSWORD_NOT_MATCHING);
